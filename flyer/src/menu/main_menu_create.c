...
void main_menu_create(menu_t* m){
	m->n_items = 4;
	m->items = (menu_item_t*)malloc(sizeof(menu_item_t)*4);
...
//	strcpy( m->items[1].image_file_name, "data/menu/nowa_gra.bmp");
	...
	m->items[2].parent = m;
	//strcpy( m->items[2].image_file_name, "data/menu/nowa_gra.bmp");

	m->items[3].parent = m;
//	strcpy( m->items[3].image_file_name, "data/menu/nowa_gra.bmp");
}
